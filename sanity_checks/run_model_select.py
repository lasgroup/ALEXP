...
from plotting.plot_specs import color_dict, line_color, shade_color, label, linestyles

#sample the environment
feature_map = CombOfLegendreMaps(num_dim_x=1, sparsity=3, max_degree=6)
...
for run in range(runs):
    ...
    algo = LEXP(domain = domain, feature_map = feature_map, T = T, likelihood_std =  likelihood_std,
                 banditalg = 'UCB', lambda_coef = lambda_coef)
...
axes[0].plot(np.mean(simple_regrets, axis=0), color=line_color['select'], label=label['select'])
...
axes[1].plot(np.mean(cum_regrets, axis=0), color=line_color['select'], label=label['select'])
...
for run in range(runs):
    ...
    cum_regret_bp = np.cumsum(regret_bp, axis=-1)
    simple_regrets.append(simple_regret)
    cum_regrets.append(cum_regret_bp)
