...
def main(args):
    ...
    if args.fixed_oracle:
        rds_env = np.random.RandomState(args.seed_env)
    ...
    if args.fixed_oracle:
        true_model = rds_env.randint(0, true_kernel.num_groups)
    else:
    ...
    if args.oracle_or_lasso == 'oracle':
        algo = LEXP(domain=domain, feature_map=feature_map, T=args.horizon, likelihood_std=args.likelihood_std,
                gammaEXP=args.gammaEXP,etaEXP=args.etaEXP,random_state=rds, adaptive=args.adaptive,
    ...
    for t in range(args.horizon):
        ...
        x, x_bp = algo.select_virtual(true_model)
        evaluation = env.evaluate(x, x_bp=x_bp)
        evals_virtual.append(evaluation['y_exact'])
    ...
    evals_stacked['y_virtual'] = evals_virtual
...
if __name__ == '__main__':
    ...
    parser.add_argument('--oracle_or_lasso', type=str, default='lasso', help = 'runs LEXP with oracle reward estimator')
